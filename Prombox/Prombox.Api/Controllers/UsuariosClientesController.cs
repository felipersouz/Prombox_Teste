...
namespace Prombox.Web.Controllers
{
    ...
    [Authorize("Bearer")]
    public class UsuariosClientesController : BaseController
    {
        private const int PAGE_SIZE = 50;
        private readonly IUsuariosClientesService _service;

        public UsuariosClientesController(IUsuariosClientesService service)
        {
            _service = service;
        }
        ...
        [HttpPost]
        public ActionResult Post([FromBody] UsuarioClienteCreateCommand command)
        {
            if (command == null)
                return BadRequest();
        ...
        [ProducesResponseType(400)]
        [HttpPut("{id}")]
        public ActionResult Put(long id, [FromBody] UsuarioClienteUpdateCommand command)
        {
            if (command == null)
                return BadRequest();
        ...
        public ActionResult Delete(long id)
        {
            ...
            var result = _service.Delete(new UsuarioClienteDeleteCommand { Id = id });
