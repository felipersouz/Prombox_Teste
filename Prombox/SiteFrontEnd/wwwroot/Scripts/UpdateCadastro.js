jQuery(document).ready(function () {
    ...
    $("#btnAlterarSenha").click(function () {
        ...
        //$("#AlterarSenhaModal").modal({ 'backdrop': 'static', 'keyboard': false, 'show' : true });        
    });
    $("#confirmAlterarSenha").click(function () {
    ...
    //senha validate
    jQuery("#formAlterarSenha").validate({
        rules: {
            senha: {
                required: true,
                rangelength: [6, 12],
    ...
    jQuery.validator.addMethod("pwcheck2", function () {
        if ($("#senha").val() == $("#senhaAtual").val())
            return false;
        else return true;
    },
        "A senha não pode ser igual a anterior"
    );
    ...
    $("#rg").mask("00.000.000-0", optionsOnlyNumber);
...
function alterarSenha() {
    ...
    // Validação
    var isValido = $('#formAlterarSenha').valid();
    if (!isValido) {
        return;
    } else {
    ...
    $.ajax({
        ...
        success: function (xhr, status, response) {
            $('#AlterarSenhaModal').modal('toggle');
            $('#sucessoSenhaModal').modal('show');
...
function finalizar(fake) {
    ...
    $.ajax({
        ...
        success: function (xhr, status, response) {
            ...
            $('#cadastroSucessModal').modal('toggle');
...
function MudouCheckBoxTermo() {
    if ($('#termo1').prop('checked') == false || $('#termo2').prop('checked') == false) {
